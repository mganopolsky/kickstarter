...
1. To find a dataset on which to conduct Exploratory Data Analysis (EDA), and on which to perform **machine learning tasks**.
2. To use as least 2 methods beyond linear gregression during the project.
3. To achieve an accurace of **> 50%**, and validate this with the **test data set**, as described below.
3. To summarize the findings, draw conclusions, and provide further recommendations, if needed.

The dataset I have chosen is the Kickstarter Dataset from Kaggle, use during the project beyond the manipulations suggested in the assignment; the modifications will be shown in the **Dataset** section below. 
...
An exploratory analysis will be performed to generate predicted succeses levels of the Kickstarter Project, with various models, to be specified; a best-performing algorithm will be chosen, and the results of the calculations will be analyzed and explained; finally a conclusion will be provided.
